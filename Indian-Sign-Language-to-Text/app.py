    ...
    while True:
        ...
        if results.multi_hand_landmarks is not None:
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks,
                ...
                # Calculate the gesture ID that appears most frequently among the most recently detected
...
def draw_info_text(image, brect, handedness, hand_sign_text,
    ...
    global predictions
    predictions = np.append(predictions,hand_sign_text)
    print(predictions)
    # global sentence
    if(np.unique(predictions[-10:])[0]==hand_sign_text):  #here problem is, the predictions array store every frame, which is very fast
        #this if condition is never true
        if(len(sentence)>0):
            if(hand_sign_text != sentence[-1] ):
                if(hand_sign_text!="null"):
                    print('executed1')
    ...
    cv.putText(image, ' '.join(sentence), ((image.shape[1] - cv.getTextSize(hand_sign_text, cv.FONT_HERSHEY_SIMPLEX, 0.6, 1)[0][0]) // 2, image.shape[0] - 10),cv.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255),1,cv.LINE_AA)
